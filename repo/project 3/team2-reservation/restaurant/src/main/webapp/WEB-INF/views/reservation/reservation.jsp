...
<%-- <% if(!session.getAttribute("authYN").equals("y")){
 ...
 --%>
		...
		$(".seat").click(function(e) {
			...
			if (prevTarget == e.target) {
				...
				var seatNo = e.target.id;
				 alert(seatNo + ' 번 테이블을 선택하셨습니다.');
				
				var inputVal =e.target.id;
				if (prevTarget != null) {
					$(prevTarget).css({
				...
				$('input[name=resTableId]').val(e.target.id);
				// $('#resTableId').val(e.target.id);
	...
	<div class="w3l-clients" id="client" >
				...
				<div class="form-right-inf">
					<div class="form-inner-cont">
						...
						<form action="/reservation/book" method="post" class="signin-form" id="book_form">
							...
							<div class="main-titles-head ">
		<h3 class="header-name white " style="color: #000;">Reservation
			Your Table</h3>
		<br />
	</div>
	...
	<ul class="showcase">
		<li>
			<div class="box"></div> <small>선택 가능 좌석</small>
		</li>
		<li>
			<div class="box selected"></div> <small>선택 좌석</small>
		</li>
		<li>
			<div class="box occupied"></div> <small>예약 좌석</small>
		</li>
	</ul>
	
	<div class="rsv_container" style="display:none">
	<br>
	<div class="river_view"><h3>River</h3></div>
	
		<div class="row">	
			<button class="seat" id="1" style="text-align: center;">1</button>
			<button class="seat" id="2" style="text-align: center;">2</button>
			<button class="seat" id="3" style="text-align: center;">3</button>
			<button class="seat" id="4" style="text-align: center;">4</button>
			<button class="seat" id="5" style="text-align: center;">5</button>
			<button class="seat" id="6" style="text-align: center;">6</button>
			<button class="seat" id="7" style="text-align: center;">7</button>
			<button class="seat" id="8" style="text-align: center;">8</button>
			
		</div>
		<div class="row">
			<button class="seat" id="9" style="text-align: center;">9</button>
			<button class="seat" id="10" style="text-align: center;">10</button>
			<button class="seat" id="11" style="text-align: center;">11</button>
	...
	$('#resDate, #resTime').on('change', function(event) {
		...
		// const formData = $('#book_form').serialize();
		...
		$.ajax({
			...
			"success":function(data,status,xhr){
				...
				$('.rsv_container').slideDown(1000); // 테이블 버튼 활성화 아래로 내려가는 방식 
				/* $('.rsv_container').fadeIn(1000); */
				$('.seat').removeAttr('style');
			
				for (var i = 0; i < data.length; i++) {
					$('#' + data[i].resTableId).prop('disabled', true);
					$('#' + data[i].resTableId).css('background-color','#fff');
					$('#' + data[i].resTableId).css('color','#000');
